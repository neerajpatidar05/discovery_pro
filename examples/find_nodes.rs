...
async fn main() {
    ...
    let enr = {
        ...
        if let Some(ip4) = args.enr_ip4 {
            ...
            if ip4.is_unspecified() {
                ...
                info!("iffffffffffffffffffffffffffff");
            } else {
                builder.ip4(ip4).udp4(port);
                info!("elseeeeeeeeeeeeeeeeeeeeeeee");
            }
        }
  ...
  info!("enr enr enr ern enr {}",enr);
    ...
    for enr in args.remote_peer {
        info!( 
            "Remote ENR read. udp4 socket: {:?}, udp6 socket: {:?}, tcp4_port {:?}, tcp6_port: {:?},devil{:?}",
            enr.udp4_socket(),
            enr.udp6_socket(),
            enr.tcp4(),
            enr.tcp6(),
    ...
    loop {
        tokio::select! {
            _ = query_interval.tick() => {
                ...
                // let mut Hardcodednodeid=[64, 237, 89, 33, 130, 232, 121, 94, 25, 166, 194, 125, 170, 116, 1, 12, 43, 143, 87, 239, 105, 54, 233, 250, 163, 51, 84, 49, 11, 73, 170, 83];
                //  let target_random_node_id = enr::NodeId::new(&Hardcodednodeid);
      //let target_random_node_id= enr.node_id();
                info!("hey neeraj this is random node id of enr 2 :{:?}",target_random_node_id.to_string());
            // get metrics 
